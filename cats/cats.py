...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def judge(paragraph):
        removed_punctuation_p = remove_punctuation(paragraph)
        lower_p = lower(removed_punctuation_p)
        split_p = split(lower_p)
        flag = False
        for p in split_p:
            if p in subject:
                flag = True
    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        ...
        for i in range(min_len):
            if typed_words[i] == source_words[i]:
                sum_len += 1
        return sum_len / len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        return res if diff_function(typed_word, res, limit) <= limit else typed_word
    # END PROBLEM 5
...
def time_per_word(words, timestamps_per_player):
    ...
    for j in range(len(tpp)):
        times.append([tpp[j][i] - tpp[j][i-1] for i in range(1, len(tpp[j]))])
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for w in word_indices:
        min_time = min(times[p][w] for p in player_indices)
        for p in player_indices:
            if times[p][w] == min_time:
                result[p].append(words[w])
                break
    return result
    # END PROBLEM 10
